// anhtrangchip's first simple game for  second semester's project in UET
...
bool loadMedia()
{
	...
	//Load texture
	gStartMenu = loadTexture( "startmenu.png" );
	if( gStartMenu == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureBox = loadTexture( "box.png" );
	if( gTextureBox == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureMar = loadTexture( "star01.png" );
	if( gTextureMar == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureGalaxy = loadTexture( "galaxy.png" );
	if( gTextureGalaxy == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureCongrats = loadTexture( "Graphic-Congrats-winners.png" );
	if( gTextureCongrats == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureLevelup = loadTexture( "kisspng-paper-heart-text-box-love-frame.png" );
	if( gTextureLevelup == NULL )
	{
		printf( "Failed to load texture image!\n" );
		success = false;
	}
	gTextureGameOver = loadTexture( "gameover.png" );
